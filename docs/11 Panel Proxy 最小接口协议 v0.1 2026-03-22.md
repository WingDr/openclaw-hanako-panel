...
- 浏览器登录与基础鉴权
- 页面初始化 HTTP 接口
- chat / logs / workspace / cron 的当前协议
- 基本错误模型
- 统一 envelope 结构
...
## 10. `GET /api/workspace/:agentId/tree`

### 作用

返回当前 agent workspace 的树快照。

### 说明

- workspace 根路径解析顺序：
  - Gateway agent catalog 中的 `workspace`
  - `OPENCLAW_CONFIG_DIR/openclaw.json` 的 `agents.list[*].workspace`
  - `agents.defaults.workspace`
- proxy 会拒绝越界路径访问

## 11. `GET /api/workspace/:agentId/file`
...
- 二进制文件拒绝读取
- 超大文件拒绝内联编辑
- 返回 `content`、`size`、`updatedAt`

## 12. `PUT /api/workspace/:agentId/file`
...
- 仅允许写回 workspace 根目录内的已存在文件
- 写回后返回新的文档快照

## 13. `POST /api/cron/validate`

### 作用

对 cron `job` 或 `patch` 做 proxy 侧 schema 校验。

### 说明

- 结构化 UI 提交前会调用
- JSON 模式也会调用
- 失败时返回 `cron_validation_failed`

## 14. `GET /api/cron`

### 作用

列出 cron jobs，可按 `agentId` 过滤。
...
## 16. `PATCH /api/cron/:jobId`

### 作用

局部更新 cron job。

## 17. `DELETE /api/cron/:jobId`

### 作用

删除 cron job。

## 18. `POST /api/cron/:jobId/run`

### 作用

立即触发一次 cron job。

## 19. `POST /api/cron/:jobId/toggle`

### 作用

快速启停 cron job，body 形如：
...
## 20. `chat.inject` WebSocket 命令

### 作用

把外部上下文注入当前 chat session。

### 当前用法

- 右侧 workspace 文件添加到聊天
- 注入正文前会带 `File: <relative path>` 标注
- 支持整文件注入和选中片段注入
